...
namespace LeoBase
{
    public partial class DrawingBase : UserControl, IDisplay<HObject>
    {
        public DrawingBase()
        {
            InitializeComponent();
        ...
        private void Zoom(ZoomMode type)
        {
            ...
            if (zoom > ResetZoom / 3)
            {
                ...
                hWindowControl1.HalconWindow.DispObj(new HObject(H_Region));
            }
        }
        ...
        private void hWindowControl1_HMouseMove(object sender, HMouseEventArgs e)
        {
            ...
            if (gray.Length == 1)
            {
                ...
                if (a.S.Equals("temperature"))
                {
                    tsslGray.Text = string.Format("{0:0.000}", gray[0].D);
                }
                else
                    tsslGray.Text = string.Format("{0:000}", gray[0].D);
            }
            ...
            hWindowControl1.HalconWindow.DispObj(H_Region);
        }


        ///add leo 2018.8.21-10.:55
        ///

        private void DrawingBase_Load(object sender, EventArgs e)
        {
            H_Region.GenEmptyRegion();
            H_TempRegion.GenEmptyRegion();
        }

        //绘制完成调用事件
        public HDrawingObject.HDrawingObjectCallback hDrawingCallBack;//Draw事件

        //当前ROI操作
        public RoiOperation _RoiOperation = RoiOperation.New;

        public HRegion H_Region = new HRegion();

        public HRegion H_TempRegion = new HRegion();

        //回调函数
        private void DrawingEventHandler(IntPtr p_draw, IntPtr p_window, string str_type)
        {
            iDrawTimer = 0;
        }
        ...
        private void Btn_Draw_Click(object sender, EventArgs e)
        {
            ...
            HTuple hv_Row, hv_Col, hv_Width, hv_Height;

            HOperatorSet.GetImageSize(SourceImage, out hv_Width, out hv_Height);
            HOperatorSet.AttachBackgroundToWindow(SourceImage, DisplayHandle);
            hWindowControl1.HalconWindow.SetDraw("margin");
            hWindowControl1.HalconWindow.SetColor("blue");
            if (H_Region != null ? H_Region.Area > 0 : false)
            {
                hWindowControl1.HalconWindow.DispObj(H_Region);
            }
            hv_Row = hv_Height / 2;
            ...
            switch (tsb.Name)
            {
                case "Btn_DrawRectangle1":
                    hDrawObject.CreateDrawingObjectRectangle1(hv_Row - hv_Height / 20, hv_Col - hv_Width / 20,
                            hv_Row + hv_Height / 20, hv_Col + hv_Width / 20);
                    break;
                case "Btn_DrawRectangle2":
                    hDrawObject.CreateDrawingObjectRectangle2(hv_Row, hv_Col, new HTuple(0.3), hv_Width / 20, hv_Height / 20);
                    break;
                case "Btn_DrawCircle":
                    hDrawObject.CreateDrawingObjectCircle(hv_Row, hv_Col, hv_Height / 10);
                    break;
                case "Btn_DrawEllipse":
                    hDrawObject.CreateDrawingObjectEllipse(hv_Row, hv_Col, new HTuple(0.2), hv_Width / 20, hv_Height / 20);
                    break;
                case "Btn_DrawRegion":
                    break;
                case "Btn_DrawLine":
            ...
            hDrawObject.SetDrawingObjectParams("color", "green");
            hDrawingCallBack = new HDrawingObject.HDrawingObjectCallback(DrawingEventHandler);
            hDrawObject.OnDrag(hDrawingCallBack);
            hDrawObject.OnResize(hDrawingCallBack);
            hDrawObject.OnSelect(hDrawingCallBack);
            hWindowControl1.HalconWindow.AttachDrawingObjectToWindow(hDrawObject);
        }
        #endregion

        #region 选择ROI操作
        private void RoiOperation_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (RoiOperationCbox.SelectedItem.ToString())
            {
                case "新建":
                    _RoiOperation = RoiOperation.New;
                    break;
                case "合并":
                    _RoiOperation = RoiOperation.Union;
                    break;
                case "差集":
                    _RoiOperation = RoiOperation.Difference;
                    break;
                case "交集":
                    _RoiOperation = RoiOperation.Intersection;
                    break;
                case "对称差":
                    _RoiOperation = RoiOperation.Xor;
                    break;
                default:
                    break;
            }
        ...
        private void DrawTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                iDrawTimer = 0;
                switch (_RoiOperation)
                {
                    case RoiOperation.New:
                        H_Region = new HRegion(hDrawObject.GetDrawingObjectIconic());
                        break;
                    case RoiOperation.Union:
                        H_TempRegion = new HRegion(hDrawObject.GetDrawingObjectIconic());
                        H_Region = H_Region.Union2(H_TempRegion);
                        break;
                    case RoiOperation.Difference:
                        H_TempRegion = new HRegion(hDrawObject.GetDrawingObjectIconic());
                        H_Region = H_Region.Difference(H_TempRegion);
                        break;
                    case RoiOperation.Intersection:
                        H_TempRegion = new HRegion(hDrawObject.GetDrawingObjectIconic());
                        H_Region = H_Region.Intersection(H_TempRegion);
                        break;
                    case RoiOperation.Xor:
                        H_TempRegion = new HRegion(hDrawObject.GetDrawingObjectIconic());
                        H_Region = H_Region.SymmDifference(H_TempRegion);
                        break;
                    default:
                        break;
                }
                //hWindowControl1.HalconWindow.AttachBackgroundToWindow(new HImage(SourceImage));
                hWindowControl1.HalconWindow.ClearWindow();
                hWindowControl1.HalconWindow.DetachDrawingObjectFromWindow(hDrawObject);
                hWindowControl1.HalconWindow.SetDraw("margin");
                hWindowControl1.HalconWindow.SetColor("blue");
                hWindowControl1.HalconWindow.DispObj(H_Region);
                DrawTimer.Stop();
        ...
        HDrawingObject hDrawObject = new HDrawingObject();
        private void hWindowControl1_HMouseDown(object sender, HMouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right && InteractiveEnable)
            {
                if (isFirst)
                {
                    timer.Enabled = true;
